...
在运行真机桥的开发机上打开 `http://127.0.0.1:18117`，或从同一局域网访问 `http://<开发机局域网地址>:18117`。模拟器按照公开的 Cardputer Zero 外观与 46 键布局呈现 320×170 界面，并连接开发机上的 USB 接收器，提供实时频谱、频率/增益控制和 76–108 MHz 的 WFM 广播音频。浏览器要求用户点击“播放真机声音”后才能出声。该服务只用于可信局域网或私有 Tailnet，不通过公网反向代理发布，也不替代实体 Cardputer Zero 的 P0 发布验证。
