...
| Keys | Exercise F/X/Z/C, Enter, Esc, G/M/L/T, number row, period, Backspace, and hold-to-repeat | one action per press; repeat is controlled; Esc returns safely; privacy-safe diagnostics cover navigation, confirm/back, shortcuts, direct entry, and repeat without key values | Pending device |
...
| USB power | Capture continuously with screen/audio active | BQ27220/BQ27 remains present with a valid charging state; capacity/voltage/current/temperature stay readable and within the official driver ranges; no brownout, disconnect loop, or thermal warning | Pending device |
...
The result schema must be `zero-sdr-p0-result-v5` and record the actual resource
sampling interval. Every resource row must also contain valid board-battery
presence, charging status, capacity, voltage, current, and temperature values;
missing telemetry or a mid-run absent battery invalidates the power evidence
...
- positive, internally consistent navigation, confirm/back, shortcut, and direct-entry key-group counters plus at least one repeat event;
